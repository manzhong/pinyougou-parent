...
app.controller('itemCatController' ,function($scope,$controller ,typeTemplateService,itemCatService){
    ...
    //设置面包屑  当第一次点击查询下级时 grade=2 if走2 面包屑第二栏有值 当第二次点击查询下级时 grade=3 走if3
    //面包屑第三栏有值 并隐藏查询下级
    //当点击 面包屑第二栏
    ...
    //下拉选矿数据源
	$scope.type_templateList={data:[]};
	$scope.findType_TemplateList=function () {
		typeTemplateService.selectOptionList().success(
			function (response) {
                $scope.type_templateList={data:response}
            }
		)
    }
